...
let client_balance = [10000, 10000, 50000, 200, 200];
...
setTimeout(() => {
  ...
  try {
    ...
    client_balance[4] -= 101;
    client_balance[3] += 100;
  } catch (error) {
    let time = new Date().toLocaleTimeString();
...
setTimeout(()=>{
  ...
  let flag = (alice.availableGold + bob.availableGold + charlie.availableGold + minnie.availableGold + mickey.availableGold) % 25 === 0;
  console.log(`Minnie's balance is ${minnie.availableGold}.(${flag && minnie.availableGold >= client_balance[3]? '\x1b[32mcan match\x1b[0m': '\x1b[31mcannot match\x1b[0m'} with the expected balance ${client_balance[3]} plus mining reward and transaction fees)`);
  console.log(`Mickey's balance is ${mickey.availableGold}.(${flag && mickey.availableGold >= client_balance[4]? '\x1b[32mcan match\x1b[0m': '\x1b[31mcannot match\x1b[0m'} with the expected balance ${client_balance[4]} plus mining reward and transaction fees)`);
  console.log();
  console.log(record);
  let elapsedTime = (alice.lastConfirmedBlock.timestamp - startTime) / 1000;
  let min_block = elapsedTime * (Blockchain.BLOCKS_BETWEEN_UPDATES / (Blockchain.TIME_BETWEEN_UPDATES / 1000)) / 2;
  let max_block = elapsedTime * (Blockchain.BLOCKS_BETWEEN_UPDATES / (Blockchain.TIME_BETWEEN_UPDATES / 1000)) * 2;
  console.log(`Executed for ${elapsedTime} seconds.`);
  console.log(`Expected number of blocks: ${min_block.toFixed(0)} ~ ${max_block.toFixed(0)}`);
